...
class TLSConnection(TLSRecordLayer):
    ...
    def handshakeServer(self, verifierDB=None,
                        ...
                        nextProtos=None, anon=False, skipTls=False):
        ...
        for result in self.handshakeServerAsync(verifierDB,
                ...
                nextProtos=nextProtos, anon=anon, skipTls=skipTls):
            pass
    ...
    def _handshakeServerAsyncHelper(self, verifierDB,
        ...
        # For skipTls, unknown how resumption would be handled. TODO: check and kill it if so
        ...
        # Perform the RSA key exchange
            ...
            print("_handshakeServerAsyncHelper: Performing RSA Key exchange")
        ...
        print("_handshakeServerAsyncHelper: RSA key exchange finished.")
    ...
    def _serverGetClientHello(self, settings, certChain, verifierDB,
        ...
        elif certChain and not skipTls:
            cipherSuites += CipherSuite.getCertSuites(settings)
        elif certChain and skipTls:
            print("In _serverGetClientHello, CipherSuite.getCertSuitesSkipTls(settings): ", 
                    CipherSuite.getCertSuitesSkipTls(settings))
            cipherSuites += CipherSuite.getCertSuitesSkipTls(settings)
        elif anon:
            cipherSuites += CipherSuite.getAnonSuites(settings)
        else:
            assert(False)

        print("In _serverGetClientHello, cipherSuites on my end: ", cipherSuites)
        ...
        #Same thing for skipTls
        #If an RSA suite is chosen, check for certificate type intersection
        if cipherSuite in CipherSuite.skipTlsSuites and CertificateType.x509 \
                            not in clientHello.certificate_types:
            for result in self._sendError(\
                    AlertDescription.handshake_failure,
                    "the client doesn't support my certificate type"):
    ...
    def _serverCertKeyExchange(self, clientHello, serverHello, 
        ...
        # Damn. Certificate class is only appending 1 cert.
        ...
        # If doing skipTls, do not send ServerHelloDone()
        if not skipTls:
            msgs.append(ServerHelloDone())

        print("_serverCertKeyExchange: Sending ServerHello, Certs")
        for result in self._sendMsgs(msgs):
            yield result
        print("_serverCertKeyExchange: Messages sent. Continuing...")
        ...
        # Atredis: set master secret to zero
    ...
    def _serverFinished(self,  premasterSecret, clientRandom, serverRandom,
                        cipherSuite, cipherImplementations, nextProtos, skipTls):

        if skipTls:
            masterSecret = None
            print("In _serverFinished, skipTls: %s" % skipTls)
            for result in self._sendFinished(masterSecret, skipTls=skipTls):
                yield result
        else:
            masterSecret = calcMasterSecret(self.version, premasterSecret,
                                          clientRandom, serverRandom)
    ...
    def _sendFinished(self, masterSecret, nextProto=None, skipTls=False):
       
        print("In _sendFinished, skipTls: %s" % skipTls)
        if not skipTls:
        ...
        verifyData = self._calcFinished(masterSecret, True, skipTls=skipTls)
    ...
    def _calcFinished(self, masterSecret, send=True, skipTls=False):
        print("In _calcFinished, skipTls: %s" % skipTls)
        if self.version == (3,0):
            ...
            if skipTls:
                # temporary
                verifyData = PRF_skipTls(label, handshakeHashes, 12)
                print("_calcFinished, PRF_skipTls(label, handshakeHashes, 12):", verifyData)
            else:
