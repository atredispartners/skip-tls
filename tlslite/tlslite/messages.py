...
class Certificate(HandshakeMsg):
    def __init__(self, certificateType):
        print("class Certificate, __init__")
        HandshakeMsg.__init__(self, HandshakeType.certificate)
        self.certificateType = certificateType
        self.certChain = None

    def create(self, certChain):
        print("class Certificate, create")
        self.certChain = certChain
        print("class Certificate, create, certChain: %s" % certChain)
        return self

    def parse(self, p):
        print ("class Certificate, parse")
        p.startLengthCheck(3)
        if self.certificateType == CertificateType.x509:
            chainLength = p.get(3)
            print("Certificate class, chainLength: ", chainLength)
            index = 0
            certificate_list = []
