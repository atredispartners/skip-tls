...
def P_hash_skipTls(macFunc, seed, length):
    bytes = bytearray(length)
    A = seed
    index = 0
    secret = ''
...
def PRF_skipTls(label, handshakeHashes, length):
    ...
    p_md5 = P_hash_skipTls(HMAC_MD5, label + handshakeHashes, length)
    p_sha1 = P_hash_skipTls(HMAC_SHA1, label + handshakeHashes, length)
...
def calcMasterSecret(version, premasterSecret, clientRandom, serverRandom, skipTls):
    ...
    if skipTls:
        if version == (3,0):
            ...
            masterSecret = PRF_broken(premasterSecret, b"master secret",
                                clientRandom + serverRandom, 48)
        elif version == (3,3):
            masterSecret = PRF_1_2_broken(premasterSecret, b"master secret",
                                clientRandom + serverRandom, 48)
        else:
