...
def handleArgs(argv, argString, flagsList=[]):
    ...
    if "skipTls" in flagsList:
        retList.append(skipTls)
    return retList
...
def serverCmd(argv):
    (address, privateKey, certChain, tacks,
        verifierDB, directory, reqCert, skipTls) = handleArgs(argv, "kctbvd", ["reqcert", "skipTls"])

    if (certChain and not privateKey and not skipTls) or (not certChain and privateKey):
        raise SyntaxError("Must specify CERT and KEY together")
    if tacks and not certChain:
        raise SyntaxError("Must specify CERT with Tacks")

    print("I am a Skip TLS HTTPS test server, listening on %s:%d" %
            (address[0], address[1]))
    if directory:
        os.chdir(directory)
    ...
    if certChain and not privateKey:
        print("Using certificate with no private key -- skip-tls")
        skipTls=True
        ...
        def handshake(self, connection):
            ...
            try:
                ...
                connection.handshakeServer(certChain=certChain,
                                              ...
                                              sessionCache=sessionCache,
                                              settings=settings,
                                              nextProtos=[b"http/1.1"], skipTls=skipTls)
    ...
    httpd = MyHTTPServer(address, CustomHTTPRequestHandler)
